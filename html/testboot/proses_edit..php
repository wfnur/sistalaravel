...
$pilih_data=mysql_query("select * from data_matkul where kode_matkul='$kod' and prodi='$prodi'");
while ($isi=mysql_fetch_array($pilih_data)){;
$sms=$isi['semester'];
$nip2=$_SESSION['username'];
$kod2=$_GET['mk'];
if($uas<=100 or $uts<=100 or $harian<=100){	
	$simpan=mysql_query("UPDATE data_nilai SET harian='$harian',uts='$uts',uas='$uas',akhir='$akhir',indeks='$indeks',indeks2='$indeks2' where nim2='$nim' and kode_dosen='$nip2' and kode_matkul2='$kod2'");
	
	if ($simpan)  { ?>
		<div class="alert alert-info">Nilai Berhasil Diubah <a href="javascript:history.go(-2)">kembali ke halaman nilai </a></div>
	<?php } else { ?>
