...
<?php 
	   ...
	   if ((isset($_SESSION['username'])) and ($_SESSION['status']=='dosen'))   { $asem=$_SESSION['username'];?>
...
$ambildata=mysql_query("SELECT * FROM `data_pengajar` inner JOIN data_matkul ON data_matkul.prodi=data_pengajar.prodi and data_matkul.kode_matkul=data_pengajar.kode_matkul where data_pengajar.kode_dosen='$asem'");
...
if ($jumlah==0){?>
	  <a href="?menu=inbobot&mk=<?php echo $kod_mk ?>">Tambahkan Bobot</a></td>
	  
  <?php }
else{?> <a href="?menu=dolin&mk=<?php echo $kod_mk ?>&prodi=<?php echo $prodi ?>&kelas=<?php echo $kelas ?>">Lihat Nilai</a></td><?php}?>
      </p>
