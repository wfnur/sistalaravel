...
$kod=$_GET['mk'];
	...
	if($total==100){
	$simpan=mysql_query("Update data_bobot set harian='$harian',uts='$uts',uas='$uas' where kode_matkuls='$kod'and nip2='$nip'");
	if ($simpan)  {
		echo "<script> alert ('Bobot Nilai Berhasil Dirubah');</script>";
	} else {
		echo "<script> alert ('Gagal'); </script>";
		echo $harian.'-'.$uts.'-'.$uas.'-'.$kod.'-'.$nip; 
	}
	}
