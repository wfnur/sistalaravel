	...
	<body>
		...
		<div class = "container-fluid" style = "margin-top:70px;">
			...
			<div class = "alert alert-info">Rekap</div>
				<center><h2>Rekap Data Absensi <?php echo $_POST['kelas_dipilih']?></center></h2></br>
		...
		<table width="99%" border="0" class="datatable">
			...
			$query=mysql_query("SELECT * FROM `dosen` WHERE `dosen_id` = '$_SESSION[dosen_id]'",$koneksi);
			
			while($row=mysql_fetch_array($query))
			{
				?><option value="<?php  echo $row['prodi_id']; ?>" selected="selected"><?php  echo $row['prodi_id']; ?></option><?php 
			}
			?>
			</select>	
		  </td>
			...
			while($row=mysql_fetch_array($query))
			{
				?><option value="<?php  echo $row['kelas_id']; ?>" selected="selected"><?php  echo $row['kelas_id']; ?></option><?php 
			}
			?>
			</select>	
		  </td>
